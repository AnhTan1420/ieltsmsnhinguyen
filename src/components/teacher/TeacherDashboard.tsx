...
/**
 * Kiểm tra một bài nộp đã được chấm điểm hay chưa. Dùng thống nhất trong toàn bộ
 * component thay vì check rải rác `submission.band_score != null` ở nhiều nơi —
 * tránh sai lệch logic nếu sau này đổi tiêu chí "đã chấm".
 */
function hasBeenGraded(submission: Pick<SubmissionRow, "band_score">): boolean {
...
export default function TeacherDashboard() {
  ...
  // Số bài đã nộp (status === "completed") nhưng CHƯA có điểm — dùng cho các badge/counter
  // "chờ chấm" ở sidebar và tab. Chỉ đếm, không tự động kích hoạt chấm điểm.
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "submissions" }, () => {
        // ⚠️ CHẶN AUTO-GRADE: callback này CHỈ được phép làm mới danh sách bài nộp để
        // cập nhật UI theo thời gian thực. TUYỆT ĐỐI không gọi handleGrade() hay
        // fetch("/api/grade") ở đây — việc chấm điểm chỉ được thực hiện khi giáo viên
        // chủ động bấm nút chấm bài (xem handleGrade bên dưới).
  ...
  // ⚠️ Đây là ĐIỂM DUY NHẤT trong component được phép gọi API chấm điểm — và nó chỉ
  // chạy khi giáo viên chủ động bấm nút (xem cụm nút "AI Chấm..." và banner nhắc chấm
  // thủ công bên dưới). Không có nơi nào khác (bao gồm cả Realtime callback ở trên)
  // được phép gọi hàm này một cách tự động.
            ...
            <div className="flex gap-2 rounded-xl bg-slate-900/80 p-1.5 border border-slate-700/50 backdrop-blur-md w-fit">
              ...
              >
                ...
                {pendingGradingCount > 0 && (
                  <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-orange-500 text-white animate-pulse">
            ...
            <div className="rounded-3xl bg-white p-5 shadow-sm border border-slate-200/60 sticky top-6 max-h-[85vh] flex flex-col">
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                  ...
                  {pendingGradingCount > 0 && (
                    <span className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-orange-100 text-orange-700 border border-orange-200 animate-pulse">
                      {pendingGradingCount} chờ chấm
                    </span>
                  )}
                </h2>
              ...
              <div className="space-y-3 overflow-y-auto pr-2 pb-2 custom-scrollbar mt-3">
                ...
                {submissions.map((submission) => {
                  // Bài đã nộp (completed) nhưng chưa có điểm → cần giáo viên chấm thủ công
                  const isPendingGrading = submission.status === "completed" && !hasBeenGraded(submission);
                  ...
                  <div key={submission.id} className="relative">
                    ...
                    <button
                      ...
                      className={`w-full text-left p-4 rounded-2xl border transition-all duration-200 group ${selectedSubmission?.id === submission.id && !selectionMode
                          ...
                          : selectionMode && selectedIds.has(submission.id)
                            ...
                            : isPendingGrading
                              ? "border-orange-200 bg-orange-50/60 hover:border-orange-300 hover:bg-orange-50 hover:-translate-y-0.5 hover:shadow-sm"
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {isPendingGrading ? (
                          <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-orange-500 text-white border border-orange-500 shadow-sm flex items-center gap-1 animate-pulse">
            ...
            <div className="rounded-3xl bg-white shadow-sm border border-slate-200/60 overflow-hidden">
              {!selectedSubmission ? (
                ...
                <div className="flex flex-col h-full">
                  {/* Banner nhắc chấm thủ công — chỉ hiện khi bài đã nộp (completed) nhưng
                      CHƯA có điểm. Đây là điểm nhấn quan trọng nhất: nhắc giáo viên rằng
                      điểm số KHÔNG được tạo tự động, đồng thời cho phép chấm nhanh ngay
                      tại đây mà không cần cuộn xuống. */}
                  {selectedSubmission.status === "completed" && !hasBeenGraded(selectedSubmission) && (
                    <div className="p-5 bg-gradient-to-r from-orange-50 to-amber-50 border-b border-orange-200/80">
                      <div className="flex flex-col md:flex-row md:items-center gap-4">
                        <div className="flex items-start gap-3 flex-1">
                          ...
                          <div>
                            <p className="text-sm font-black text-orange-900">Bài chờ chấm điểm</p>
                            <p className="text-sm font-medium text-orange-800/90 mt-0.5">
                              Học sinh đã hoàn thành và nộp bài. Điểm số sẽ không được tạo tự động — chỉ chấm khi giáo viên xác nhận.
                            </p>
                          </div>
                        </div>

                        {/* Nút chấm nhanh ngay trong banner — không cần cuộn xuống */}
                        <div className="flex items-center rounded-xl bg-slate-900 border border-slate-800 overflow-hidden shadow-sm shrink-0">
                          <button
                            onClick={() => handleGrade(selectedSubmission, "both")}
                            disabled={isGrading || !selectedSubmission.content}
                            className="flex items-center gap-2 bg-slate-950 hover:bg-slate-900 text-white px-4 py-2.5 text-sm font-bold transition disabled:opacity-50"
                            title="Chấm cả hai bài cùng lúc và tính trung bình cộng điểm Overall"
                          >
                            <Bot className="h-4 w-4 text-cyan-400" /> Cả 2 Task
                          </button>
                          <button
